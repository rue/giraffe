...
describe "Existing editable page" do
  ...
  it "presents the current raw content of the page in an editable section" do
    inputs = Nokogiri::HTML.parse(get("/editable/file1").body).css("#forms > form > textarea")
  ...
  it "has a button with which to submit page text" do
    inputs = Nokogiri::HTML.parse(get("/editable/file1").body).css("#forms > form input")
    inputs.find {|i| i["type"] == "submit" }.should_not == nil
  end

  it "will POST to the page resource" do
    form = Nokogiri::HTML.parse(get("/editable/file1").body).css("#forms > form").first
  ...
  it "contains links to go to page resource or to page history" do
    links = Nokogiri::HTML.parse(get("/editable/file1").body).css "a"

    links.find {|link| link["href"] == "/file1" }.should_not == nil
    links.find {|link| link["href"] == "/changes/file1" and link.content =~ /history/i }.should_not == nil
  end
end
...
describe "Nonexisting editable page" do
  ...
  it "presents an empty content input section" do
    inputs = Nokogiri::HTML.parse(get("/editable/file0").body).css("#forms > form > textarea")
    current = inputs.find {|i| i["name"] == "contents" }
  ...
  it "has a button with which to submit page text" do
    inputs = Nokogiri::HTML.parse(get("/editable/file0").body).css("#forms > form input")
    inputs.find {|i| i["type"] == "submit" }.should_not == nil
  end

  it "will PUT to the page resource (we cheat by using POST with PUT in hidden _method)" do
    form = Nokogiri::HTML.parse(get("/editable/file0").body).css("#forms > form").first

    form["method"].should == "POST"
    form["action"].should == "/file0"

    form.css("input.http_method").first["name"].should == "_method"
    form.css("input.http_method").first["value"].should == "PUT"
  end

  it "does not contain links to go to page resource or to page history" do
    links = Nokogiri::HTML.parse(get("/editable/file0").body).css "a"

    links.find {|link| link["href"] == "/file0" }.should == nil
    links.find {|link| link["href"] == "/changes/file0" }.should == nil
  end
end
...
describe "Editable page in a subdirectory" do
  ...
  it "presents the page's current content in an editable field" do
    inputs = Nokogiri::HTML.parse(get("/editable/subdir/file5").body).css("#forms > form > textarea")
    current = inputs.find {|i| i["name"] == "contents" }
  ...
  it "includes the correct directory prefixes" do
    editable = get("/editable/subdir/file5").body
    ...
    links.find {|link| link["href"] == "/changes/subdir/file5" and link.content =~ /history/i }.should_not == nil
  end
end
